...
from data_.models import TopicQueryParams, Category, User, VisibilityAuth, AccessRevocation
from services.category_service import get_all_categories, create, user_access_state
from services.category_service import get_topics_for_category, grant_category_read_access, grant_category_write_access, revoke_category_read_or_write_access, get_privileged_users
...
async def give_read_access(category_id: int, user_id: int, admin: Annotated[User, Depends(get_current_admin_user)]):
    if not admin:
...
async def give_write_access(category_id: int, user_id: int, admin: Annotated[User, Depends(get_current_admin_user)]):
    if not admin:
        return Response(status_code=401, content='You are not authorized!')
...
async def revoke_access(category_id: int, user_id: int, access: AccessRevocation, admin: Annotated[User, Depends(get_current_admin_user)]):
    if not admin:
        return Response(status_code=401, content='You are not authorized!')
