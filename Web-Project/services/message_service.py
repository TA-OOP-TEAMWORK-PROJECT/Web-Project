...
def create_message(message_data: MessageCreate, token: str):
    ...
    message_id = insert_query(
        ...
        (message_data.content, user_id, message_data.receiver_id, datetime.now())
    )
...
def get_conversation(user_id: int, token: str):
    ...
    messages = read_query(
        "SELECT * FROM message WHERE (sender_id = %s AND receiver_id = %s) OR (sender_id = %s AND receiver_id = %s)",
